...
export const useLoanFormLogic = () => {
  ...
  const defaultValues = useMemo(() => {
    ...
    const studyMaintenanceLoan = getMaintenanceLoanForSituation(
      fees,
      values.livingSituation,
    );
    const placementMaintenanceLoan = getPlacementMaintenanceLoanForSituation(
      fees,
      values.livingSituation,
    );
    ...
    const maintenanceLoan = hasPlacement
      ? studyMaintenanceLoan * (values.courseLength - 1) +
        placementMaintenanceLoan
      : studyMaintenanceLoan * values.courseLength;
    const maintenanceGrant = hasPlacement
      ? fees.maintenanceGrant * (values.courseLength - 1) +
        fees.placementMaintenanceGrant
    ...
    (fieldName: string, total: number): number[] => {
      ...
      const yearCount = isPostgradField
        ? typeof values.mastersLength === "number"
          ? values.mastersLength
          : 1
        : typeof values.courseLength === "number"
          ? values.courseLength
          : 1;
      ...
      if (
        (fieldName === "tutionFeeLoan" ||
          fieldName === "maintenanceLoan" ||
          fieldName === "maintenanceGrant") &&
        values.yearInIndustry === "yes" &&
        typeof values.placementYear === "number" &&
        ...
        const placementFees = getFeesForYear(
          values.courseStartYear + placementIndex,
        );
        ...
        if (fieldName === "tutionFeeLoan") {
          ...
          placementAmount = getPlacementMaintenanceLoanForSituation(
            placementFees,
            values.livingSituation,
          );
        } else {
